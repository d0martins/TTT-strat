...
_TARA_TTT_WINNING_TIME_S = 32 * 60 + 52.17  # Team Visma | Lease a Bike


@pytest.fixture(scope="module")
def tara_stage3_result(reference_rider, calm_wind):
...
@pytest.mark.solver
def test_tara_stage3_feasible(tara_stage3_result):
    """The plan is finite and covers the whole course grid."""
    opt, res = tara_stage3_result
...
@pytest.mark.solver
def test_tara_stage3_power_higher_on_steeper_quartile(tara_stage3_result):
...
@pytest.mark.solver
def test_tara_stage3_solo_slower_than_drafting_team_ballpark(tara_stage3_result):
    ...
    """
    _opt, res = tara_stage3_result
    ratio = res.time_total_s / _TARA_TTT_WINNING_TIME_S
